...
List<Color> coloresEtiqueta1Gradientes = [
...
Color ObtenerColoresEtiqueta1Gradiente(Color etiquetaColor) {
  int index01 = coloresEtiqueta1.indexOf(etiquetaColor);
  if (index01 != -1) {
    return coloresEtiqueta1Gradientes[index01];
  } else {
    return Colors.grey;
  }
}
  ...
  try {
    if (kIsWeb) {
      ...
      if (jsonString != null) {
        ...
        versionLocal = himnosApp['version'];
        cargadoLocal = true;
      }
    } else {
    ...
    if (kIsWeb) {
      ...
      if (await Permission.storage.isGranted) {
        ...
        if (await Permission.storage.request().isGranted) {
          ...
          // Manejar el caso en que no se tenga permiso
          print('No se tiene permiso para guardar himnos');
          openAppSettings(); // Abre las configuraciones de la app para que el usuario active el permiso
        }
      }
    }
  } catch (e) {
    print('Error al guardar himnos: $e');
    // Manejar el error
    if (e is io.FileSystemException) {
      // Si el error es de sistema de archivos, intenta pedir permiso de nuevo
      if (await Permission.storage.request().isGranted) {
        guardarHimnosInterno(); // Llama a la función de nuevo
      }
    }
  }
}
...
// Comprobar si tengo la misma versión que en el repositorio
Future<String> actualizarVersionDesdeGithubEnMemoria() async {
  final urlVersion = "https://raw.githubusercontent.com/Danel20/MusicaLXS/main/versionMusicaLXS.json";
  
  final respuesta = await http.get(Uri.parse(urlVersion)).timeout(Duration(seconds: 5));
  if (respuesta.statusCode != 200) {
    return "No Disponible";
  } else {
    return jsonDecode(respuesta.body)['version'];
  }
}
...
Future<List<String>> actualizarListasDesdeGithubEnMemoria() async {
  ...
  if (!conectado) {
    mensajes.add("❌ No hay conexión a Internet. Intenta cargar listas más tarde.");
    return mensajes;
    }
  if (!conectado) { mensajes.add("❌ No hay conexión a Internet. Intenta cargar listas más tarde."); return mensajes; }
  
  try {

    final response = await http.get(Uri.parse(urlRemoto)).timeout(const Duration(seconds: 5));
...
Future<List<String>> actualizarJsonDesdeGithubEnMemoria() async {
  ...
  if (!conectado) {
    mensajes.add("❌ No hay conexión a Internet. Intenta cargar himnos más tarde.");
    return mensajes;
  }
