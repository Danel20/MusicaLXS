...
const String asistencixs_sebipca_version = "1.0.3";
...
class SupabaseService {
  ...
  // ... dentro de class SupabaseService ...
  ...
  static Future<List<AttendanceGroup>> fetchGroupsBySection(String section) async {
    ...
    try {
      ...
      // NUEVO: Ordenamiento natural para que Grupo #2 aparezca antes que Grupo #10
  ...
  // Función auxiliar para comparar nombres con números de forma natural
...
class _GroupsScreenState extends State<GroupsScreen> {
  ...
  void _checkBirthdays() {
    ...
    final List<Person> bdayPeople = [];
    for (var group in groups) {
      for (var person in group.people) {
        if (person.birthday != null) {
          final bday = person.birthday!;
          final nextBday = DateTime(now.year, bday.month, bday.day);
          final diff = nextBday.difference(DateTime(now.year, now.month, now.day)).inDays;
          if ((diff >= 0 && diff <= 7) || (diff < 0 && DateTime(now.year + 1, bday.month, bday.day).difference(DateTime(now.year, now.month, now.day)).inDays <= 7)) {
            bdayPeople.add(person);
          }
        }
      }
    }
...
class _AttendanceTableScreenState extends State<AttendanceTableScreen> {
  ...
  void _cycleStatus(Person person, DateTime date) async {
    ...
    setState(() {
      ...
      if (current == AttendanceStatus.none) person.attendance[dateKey] = AttendanceStatus.present;
      else if (current == AttendanceStatus.present) person.attendance[dateKey] = AttendanceStatus.absent;
      else if (current == AttendanceStatus.absent) person.attendance[dateKey] = AttendanceStatus.reported;
      else person.attendance[dateKey] = AttendanceStatus.none;
    });
