              ...
              IconButton(
                ...
                var misHimnos1 = List.generate(temas.length, (index1) {
                        try {
                          ...
                          return Row(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.start, children: [formatearConNotas(TextoComprobado['tema'], TextoComprobado['contenido'], NotaTextoComprobado, index1)]);
                        } catch (e) {
                          return Row(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.start, children: [Text("Tema ${temas[index1]} no encontrado")]);
                        }
                      });
                */      
                // misHimnos1.insert(0, Row(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.start, children: [Text(titulo, style: TextStyle(color: textColor1, fontWeight: FontWeight.bold, decoration: TextDecoration.underline, decorationColor: textColor1, decorationStyle: TextDecorationStyle.wavy)),]));
                //var misHimnos2 = Container(padding: EdgeInsets.all(15), decoration: BoxDecoration(color: Colors.white), child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.max, mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start, children: misHimnos1),),);
                List<Widget> misHimnos1 = [];
                for (int i = 0; i < temas.length; i += 2) {
                  List<Widget> columnaChildren = [];
                  for (int j = i; j < i + 2 && j < temas.length; j++) {
                    try {
                      var TextoComprobado = himnos.firstWhere((item1) => item1['tema'] == temas[j]);
                      int NotaTextoComprobado = notasMusicales1.indexOf(TextoComprobado['nota']);
                      tonoOffsetsPorSeccion.putIfAbsent(j, () => NotaTextoComprobado);
                      columnaChildren.add(formatearConNotas(TextoComprobado['tema'], TextoComprobado['contenido'], NotaTextoComprobado, j));
                    } catch (e) {
                      columnaChildren.add(Text("Tema ${temas[j]} no encontrado"));
                    }
                  }
                  misHimnos1.add(IntrinsicWidth(stepWidth: 500.0, child: Column(children: columnaChildren)));
                }

                var misHimnos2 = Container(
...
class ImagenCapturadaScreen extends StatelessWidget {
  final Uint8List? image; // La imagen que recibimos de la pantalla anterior

  const ImagenCapturadaScreen({Key? key, required this.image}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: image != null
            ? Image.memory(image!)  // Mostrar la imagen capturada
